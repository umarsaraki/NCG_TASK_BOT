...
def start(message):
    ...
    if not check_membership(user_id): force_join(message)
    else:
...
def check_join(call):
    ...
    if check_membership(user_id):
        user = supabase.table('users').select("*").eq('user_id', user_id).execute()
        if len(user.data) == 0: supabase.table('users').insert({"user_id": user_id, "username": call.from_user.username, "balance": WELCOME_BONUS}).execute(); bot.send_message(user_id, f"🎁 Welcome Bonus: +{WELCOME_BONUS} points credited!")
        show_menu(call.message.chat.id); bot.answer_callback_query(call.id, "Welcome! ✅")
    else: bot.answer_callback_query(call.id, "Please join both channels first", show_alert=True)

@bot.message_handler(func=lambda m: True)
def handle_all_messages(message):
    if not check_membership(message.from_user.id): force_join(message); return
...
def referral(message):
    user_id = message.from_user.id; ref_link = f"https://t.me/{bot.get_me().username}?start={user_id}"
    ref_count = supabase.table('users').select("user_id").eq('referred_by', user_id).execute(); count = len(ref_count.data)
    text = f"👥 **Your Referral Link**\n\n{ref_link}\n\nEarn **{REFERRAL_BONUS} points** for every person who joins with your link.\n\n**Total Referrals:** {count}"
    markup = InlineKeyboardMarkup(); markup.add(InlineKeyboardButton("📋 Copy Link", url=ref_link))
...
def add_payment_data(message):
    try:
        parts = [p.strip() for p in message.text.split('|')]; acc, name, bank, phone, network = parts
        ...
        del user_states[message.from_user.id]; bot.reply_to(message, "✅ Payment method saved successfully!")
    except: bot.reply_to(message, "❌ Wrong format. Send like: `Acc | Name | Bank | Phone | Network`", parse_mode="Markdown")

def withdraw(message):
    user = supabase.table('users').select("*").eq('user_id', message.from_user.id).execute().data[0]
    if not user.get('account_number'): bot.reply_to(message, "🚫 No payment method found.\nPlease go to `Add Payment` first"); return
    user_states[message.from_user.id] = "waiting_withdraw"; bot.reply_to(message, f"💸 Minimum withdrawal is {MIN_WITHDRAW} points\nEnter amount you want to withdraw:")

def process_withdraw(message):
    try:
        amount = int(message.text); user_id = message.from_user.id
        user = supabase.table('users').select("balance").eq('user_id', user_id).execute().data[0]; balance = user['balance']
        if amount < MIN_WITHDRAW: bot.reply_to(message, f"❌ Insufficient. Minimum withdrawal is {MIN_WITHDRAW} points"); return
        if amount > balance: bot.reply_to(message, f"❌ You only have {balance} points"); return
        ...
        del user_states[user_id]; bot.reply_to(message, f"✅ Withdrawal request of {amount} points submitted!\nAdmin will process it soon.")
        for admin in ADMIN_IDS: bot.send_message(admin, f"New Withdrawal Request:\nUser: {user_id}\nAmount: {amount}")
    except: bot.reply_to(message, "❌ Enter a valid number")
...
def history(message):
    ...
    if len(tasks.data) > 0:
        text += "**Tasks:**\n"
        for t in tasks.data: status = "✅ Approved" if t['status'] == "approved" else "❌ Rejected" if t['status'] == "rejected" else "⏳ Pending"; text += f"Task {t['task_id']} - {status}\n"
    if len(withdraws.data) > 0:
        text += "\n**Withdrawals:**\n"
        for w in withdraws.data: status = "✅ Paid" if w['status'] == "paid" else "⏳ Pending"; text += f"{w['amount']} points - {status}\n"
    if len(tasks.data) == 0 and len(withdraws.data) == 0: text = "You have no history yet"
    bot.reply_to(message, text, parse_mode="Markdown")

def task(message):
    tasks = supabase.table('tasks').select("*").eq('active', True).execute()
    if len(tasks.data) == 0: bot.reply_to(message, "No available task"); return
    markup = InlineKeyboardMarkup(row_width=2); buttons = []
    for t in tasks.data: buttons.append(InlineKeyboardButton(f"Task {t['id']}", callback_data=f"view_task_{t['id']}"))
    markup.add(*buttons); bot.send_message(message.chat.id, "📋 Available Tasks:", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data.startswith("view_task_"))
def view_task(call):
    task_id = int(call.data.split('_')[2]); t = supabase.table('tasks').select("*").eq('id', task_id).execute().data[0]
...
def leaderboard(message):
    ...
    for i, t in enumerate(top.data): pos = i+1; prize = PRIZES.get(pos, 0); text += f"**TOP {pos}**: User `{t['user_id']}` - {t['points_earned']} pts\nPrize: {prize:,} Naira\n"
    if len(top.data) == 0: text = "No points earned this month yet"
    bot.reply_to(message, text, parse_mode="Markdown")

@bot.message_handler(commands=['submit'])
def submit(message):
    try:
        task_id = int(message.text.split()[1]); user_id = message.from_user.id
        check = supabase.table('user_tasks').select("*").eq('user_id', user_id).eq('task_id', task_id).execute()
        if len(check.data) > 0: bot.reply_to(message, "You already submitted this task"); return
        pending_submissions[user_id] = task_id; bot.reply_to(message, f"Send screenshot as proof for Task {task_id} 📸")
    except: bot.reply_to(message, "Usage: /submit 1")

@bot.message_handler(content_types=['photo'])
def handle_photo(message):
    user_id = message.from_user.id
    if user_id not in pending_submissions: return
    task_id = pending_submissions[user_id]; file_id = message.photo[-1].file_id; username = message.from_user.username
...
def add_task(message):
    ...
    try: parts = message.text.split('|'); title, link = parts[0].replace('/addtask ', '').strip(), parts[1].strip(); supabase.table('tasks').insert({"title": title, "link": link, "points": TASK_POINTS, "active": True}).execute(); bot.reply_to(message, f"Task added: {title} - {TASK_POINTS} points")
    except: bot.reply_to(message, "Usage: /addtask Title | Link")

@bot.message_handler(commands=['pending'])
def pending_tasks(message):
    if not is_admin(message.from_user.id): return
    pending = supabase.table('user_tasks').select("*").eq('status', 'pending').execute(); text = "📋 **Pending Submissions:**\n\n"
    for p in pending.data: text += f"User: `{p['user_id']}` @{p['username']}\nTask: `{p['task_id']}`\n/approve {p['user_id']} {p['task_id']} | /reject {p['user_id']} {p['task_id']}\n\n"
    bot.reply_to(message, text, parse_mode="Markdown")

@bot.message_handler(commands=['approve'])
def approve_task(message):
    if not is_admin(message.from_user.id): return
    parts = message.text.split(); uid, tid = int(parts[1]), int(parts[2])
    ...
    month = datetime.now().strftime("%Y-%m"); supabase.table('leaderboard').upsert({"user_id": uid, "points_earned": TASK_POINTS, "month_year": month}).execute()
    bot.reply_to(message, f"Approved! +{TASK_POINTS} points added"); bot.send_message(uid, f"Task {tid} APPROVED! +{TASK_POINTS} points ✅")
